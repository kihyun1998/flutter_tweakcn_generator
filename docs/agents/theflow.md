...
| Contract ≠ defect / the layered tie-breaker decides | **#31** — the radius steps were reported as diverging from shadcn/ui, which they do. But tweakcn emits the derivation this package reproduces, and tweakcn owns the input layer, so the generated values match the CSS the user pasted at every radius. Adopting the report would have deleted the contract. The repo's one test of the numbers sat at the default radius — the single input at which the two formulas agree — so it could not have discriminated |
...
| A status must be derived from the result, not from the incidents | **#28** — the exit code read `hasFailures` (a fact about the run) where the contract promised "the theme is usable as generated" (a fact about the product), so it was wrong in *both* directions at once. One end-of-run check replaced two patches. Two pre-existing tests asserting `exitCode, 0` incidentally had frozen the bug as expected |
| A seam above the code proves nothing about it | **#27** — the lookup half was bound to a hardcoded URL, and #23's fix landed there untested. Under the same mutation an endpoint parameter goes red while an injected lookup function stays green: the injected function replaces the very code the test is meant to cover. Also the case where an issue's *own* stated premise ("there is no way to measure this") was measured false before the work started |
